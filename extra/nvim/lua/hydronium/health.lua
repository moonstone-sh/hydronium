--[[
  Hydronium Health Check Module (:checkhealth hydronium)
...
function M.check()
  report_start("Hydronium Environment & Tooling")
  ...
  -- 2. Check Hydronium core module
  local ok, hydronium = pcall(require, "hydronium")
  if ok and hydronium then
    report_ok("Hydronium library loaded successfully (v" .. tostring(hydronium._VERSION or "0.1.0") .. ")")
  else
    report_warn("Hydronium library not found in Neovim Lua package.path; ensure project root is in rtp or package.path")
  end

  -- 3. Check CLI binary bin/luax
  local init_mod = require("hydronium")
  local root = (init_mod.get_root_dir and init_mod.get_root_dir()) or vim.fn.getcwd()
  ...
  local plugin_path = root .. "/src/hydronium/luax/plugin.lua"
