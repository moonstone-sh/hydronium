...
`moonstone/hydronium-ballad` provides Ballad build plugins for Hydronium projects:
...
```sh
moon add moonstone/hydronium-ballad
```

It installs the `hydronium_ballad` Lua namespace, resolving `hydronium`,
`moonstone/hydronium-luax`, and `moonstone/hydronium-dom` automatically.
