...
**Done.** `tests/build/assets_hash_spec.lua` (7) and `tests/build/style_bundle_spec.lua` (10) cover the two plugins, each ending in a real `site.manifest` round trip. `js/tests/spa_hash_demo.browser.test.mjs` now drives its shell from the build's own `hydronium-manifest.json` and asserts, in a real browser: the rendered class is scoped (not the authored `.card`), the built stylesheet declares exactly that class (build and runtime derive it independently — if they drift, the rule silently never applies), the browser computes `outline-color: rgb(7, 113, 199)` from it, and `#logo`'s `src` is the hashed URL and serves 200 as `image/svg+xml`. Both halves were checked against a deliberately broken build, not just a passing one.

**What it found — `hydronium_dom.assets` had no browser path at all.** `configure()` calls `loadfile`, and wasmoon has no filesystem, so a static SPA could never load a manifest: every `assets.url()` returned the documented dev fallback (`/logo.svg`), which with no SSR behind it is a 404. Closed by `assets.configure_table(manifest)` plus a new `mount({ assetManifestUrl })` option that fetches the build's `hydronium-manifest.lua` alongside the chunks and `load()`s it inside the VM. It throws if nothing in the bundle requires `hydronium_dom.assets` rather than configuring nothing silently. Removing the option from the gate reproduces the original failure exactly (`'/logo.svg'` vs `'/assets/logo.3372f8b6ee.svg'`).
