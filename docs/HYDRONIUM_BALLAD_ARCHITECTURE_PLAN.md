...
**Historical counter-precedent, stated fairly**: `hydronium/create` was a separate repo despite being Hydronium-family because it has zero runtime dependency on `hydronium` (it writes template text; its deps are `alter`, `clingy`, and `ballad`). It has since moved into this repository as the `create` Orbit because its templates co-evolve with Hydronium's LUAX, DOM, HMR, and build contracts. `hydronium-ballad` depends on Hydronium's code at internal-API depth — a different packaging concern.
...
6. **`hydronium/create`'s ownership — resolved.** The generator now lives in this repository as the `create` Orbit, so framework and scaffold changes can land and be tested atomically. Its independently versioned `hydronium/create` package and `hydronium-create` executable remain separate release products.
